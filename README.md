...
- ✅ 自动回复（关键词匹配 + 默认随机回复）
- ✅ 自定义标签转 Telegram HTML 格式（引用、加粗、斜体、代码块等）
- ✅ 毫秒级延迟响应
- ✅ 黑名单功能
- ✅ 支持 Business 账号
- ✅ 配置热加载（Python 版修改文件即时生效；Workers 版修改环境变量后重新部署）
...
[
  ...
  {
    "keywords": ["你好", "您好", "👋"],
    "reply": "<jh>[AutoReply]</jh></n></n>👋 您可以称呼我为麻花<yy>自述：<lj url=\"https://mahua.uk\">这是我</lj></yy></n><yy>Telegram：<tj>5455684444</tj></yy>"
  }
]
```
...
将 default_reply 改为数组，添加更多文案即可：

```json
"default_reply": ["文案1", "文案2", "文案3"]
```

Q: Python 版和 Workers 版可以同时运行吗？
...
这份 README 已经完整了，从文件头到许可证都有。你可以直接复制保存为 `README.md`。